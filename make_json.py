...
def get_specialization_from_filename(filepath: str) -> str:
    ...
    match = re.match(r"curs_([a-zA-Z0-9_]+?)(?:-\d)?\.csv", name_lower) # Optional "-1" etc.
    if match:
    ...
    print(f"Warning: Could not determine specialization from filename: {basename}. Using 'unknown'.")
    return "unknown"
...
def process_csvs_to_json(csv_filepaths: list[str], output_json_path: str):
    ...
    for filepath in csv_filepaths:
        ...
        print(f"Processing file: {filepath} (Deduced specialization: {file_specialization})...")
        ...
        try:
            with open(filepath, mode='r', encoding='utf-8') as infile:
                ...
                try:
                    ...
                    print(f"Warning: CSV file '{filepath}' is empty or has no header. Continuing...")
                    continue
                ...
                for i, row in enumerate(reader, 1):
                    ...
                    is_context_line = match_context and all(not cell.strip() for cell in row[1:])

                    if is_context_line:
                        current_year = int(match_context.group(1))
                        current_sem = int(match_context.group(2))
                        current_anul3_module_name = match_context.group(3).strip() if match_context.group(3) else None
                        continue
                    ...
                    if current_year is None or current_sem is None:
                        print(f"Warning: Course '{title_cell}' (row {i+1} in '{filepath}') was found before Year/Semester definition. Skipping.")
                        continue
                    ...
                    if not course_id_from_csv:
                        print(f"CRITICAL ERROR: Course '{course_name}' (row {i+1} in '{filepath}') has no Acronym/ID. This field is mandatory. Course skipped.")
                        continue
                    ...
                    if dependencies_ids_str and dependencies_ids_str.lower() != "niciuna":
                        ids_list = [id_val.strip().lower() for id_val in dependencies_ids_str.split(',') if id_val.strip()]
                        prerequisites_final_ids.extend(ids_list)
                    
                    keywords = [kw.strip() for kw in row[3:7] if len(row) > 6 and kw.strip()]
                    ...
                    if current_anul3_module_name: # Year 3 module
                        course_obj["module"] = current_anul3_module_name # Renamed from "profile"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process CSV files (with IDs in dependencies column) to generate JSON. Adds 'specialization' and uses 'module'.")
    parser.add_argument("csv_files", nargs='+', help="List of input CSV files (dependencies column must contain IDs). Expected filename format: curs_specialization.csv")
